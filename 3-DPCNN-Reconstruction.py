...
missing_type = 'IM'                    # RM = Regularly Missing; CM = Consecutively Missing; IM = Irregularly(Randomly) Missing,
missing_coefficient  = 0.5             # For RM, missing 1 seismic trace for every missing_coefficient traces, eg. missing_coefficient=1.0/3.0;
                                       # For CM, the missing_coefficient is the number of consecutively missing seismic traces, eg. missing_coefficient=5;
                                       # For IM, the missing_coefficient is missing rate, eg. missing_coefficient=0.8.
...
task_name = 'R'                        # R = Reconstruction; DR = Denoising and Reconstruction
...
def train(file_path, current_train, data_dimension, dropouts_dropped_rate, data_mask_probability):
    ...
    if current_train == 1:
        ...
        missed_volume, missed_position = miss_trace(input_volume, model_path, missing_type, missing_coefficient)
    else:
        missed_volume, missed_position = read_data(file_path[0:file_path.rfind(".")] + "/" + task_name + "_train1/",data_dimension,model_path)
    model = network.PCNN_3D.build_reconstruction_net(missed_volume, missed_position, task_name, dropouts_dropped_rate, data_mask_probability)
    ...
    with tf.Session() as sess:
        ...
        for step in range(iterations):
            feet_dict = {is_reverse_polarity: np.random.randint(0, 2)}
    ...
    for file_name in files_name:
        if not os.path.isdir(data_path + file_name):
            ...
            for tr_times in range(1, train_times+1):
                ...
                best_snr_list = np.append(best_snr_list,
                                          pick_best_result(result_path, target_path, result_path+'missed_volume.bin', iterations,
                                                           test_interval))
